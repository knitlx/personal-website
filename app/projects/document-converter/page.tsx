...
const PptxInstruction = () => (
    ...
    <pre><code className="text-xs bg-gray-100 p-2 rounded-md block">
{`<!DOCTYPE html>
<html>
<head>
  <style>
...
function DocumentConverter() {
  ...
  const Input = (props: React.ComponentProps<'input'>) => (
    <input {...props} className="w-full p-2 border border-gray-300 rounded-md bg-white text-gray-900 focus:ring-2 focus:ring-primary-color focus:border-primary-color outline-none transition-all duration-200" />
  );
  
  const Select = (props: React.ComponentProps<'select'>) => (
    <select {...props} className="w-full p-2 border border-gray-300 rounded-md bg-white text-gray-900 focus:ring-2 focus:ring-primary-color focus:border-primary-color outline-none transition-all duration-200 h-[42px]" />
  );
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 w-full animate-fade-in">
            <div className="flex flex-col gap-6 col-span-1">
              <section>
                ...
                <textarea className="w-full p-3 border border-gray-300 rounded-md bg-white text-sm focus:ring-2 focus:ring-primary-color focus:border-primary-color outline-none transition-all duration-200 shadow-sm" rows={12} placeholder="Вставьте ваш HTML-код здесь..." value={htmlInput} onChange={handleHtmlInputChange} disabled={loading}></textarea>
              </section>
              <section>
                <Title>Отступы</Title>
                <div className="grid grid-cols-5 gap-3">
                  <div className="flex flex-col gap-1"><label htmlFor="top" className="text-xs font-medium text-gray-600">Верх</label><Input type="number" name="top" value={margins.top} onChange={handleMarginChange} /></div>
                  <div className="flex flex-col gap-1"><label htmlFor="right" className="text-xs font-medium text-gray-600">Право</label><Input type="number" name="right" value={margins.right} onChange={handleMarginChange} /></div>
                  <div className="flex flex-col gap-1"><label htmlFor="bottom" className="text-xs font-medium text-gray-600">Низ</label><Input type="number" name="bottom" value={margins.bottom} onChange={handleMarginChange} /></div>
                  <div className="flex flex-col gap-1"><label htmlFor="left" className="text-xs font-medium text-gray-600">Лево</label><Input type="number" name="left" value={margins.left} onChange={handleMarginChange} /></div>
                  <div className="flex flex-col gap-1"><label htmlFor="unit" className="text-xs font-medium text-gray-600">Ед.</label><Select name="unit" value={marginUnit} onChange={handleUnitChange}><option value="mm">mm</option><option value="cm">cm</option><option value="in">in</option><option value="px">px</option></Select></div>
                </div>
              </section>
              <section className="flex items-center gap-4">
                  <BentoButton onClick={handlePdfPreview} variant="outline" size="small" className="min-w-[180px] w-full"><EyeIcon />{loading ? 'Обработка...' : 'Предпросмотр'}</BentoButton>
                  <BentoButton onClick={handlePdfDownloadDirectly} variant="primary" size="small" className="min-w-[180px] w-full"><DownloadIcon/>{loading ? 'Обработка...' : 'Скачать'}</BentoButton>
              </section>
            </div>
            <div className="flex flex-col gap-4 w-full col-span-1 lg:col-span-2">
                ...
                <div className="w-full h-[60vh] min-h-[500px] lg:h-[80vh] border border-gray-200 rounded-lg flex items-center justify-center bg-gray-50 shadow-inner overflow-hidden">
                  ...
                  {error && !pdfPreviewUrl && <div className="text-red-500 text-sm p-4">{error}</div>}
        ...
        {activeTab === 'pptx' && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 w-full animate-fade-in">
            <div className="flex flex-col gap-6">
              <section>
                <Title>Вставьте HTML</Title>
                <textarea className="w-full p-3 border border-gray-300 rounded-md bg-white text-sm focus:ring-2 focus:ring-primary-color focus:border-primary-color outline-none transition-all duration-200 shadow-sm" rows={8} placeholder="Вставьте ваш HTML-код здесь..." value={htmlInput} onChange={handleHtmlInputChange} disabled={loading}></textarea>
              </section>
              <section className="flex items-center gap-4">
                <BentoButton onClick={handlePptxPreview} variant="outline" size="small" className="min-w-[180px] w-full"><EyeIcon />{loading ? 'Обработка...' : 'Предпросмотр'}</BentoButton>
                <BentoButton onClick={handleHtmlToPptxConvert} variant="primary" size="small" className="min-w-[180px] w-full"><DownloadIcon/>{loading ? 'Обработка...' : 'Скачать'}</BentoButton>
              </section>
              <section>
            ...
            <div className="flex flex-col gap-4 w-full">
              ...
              <div className="w-full min-h-[400px] lg:h-full border border-gray-200 rounded-lg flex items-center justify-center bg-gray-50 shadow-inner overflow-hidden relative">
                {loading && <div className="text-gray-500">Загрузка предпросмотра...</div>}
                {error && pptxPreviewImages.length === 0 && <div className="text-red-500 text-sm p-4">{error}</div>}
                {pptxPreviewImages.length > 0 && !error && (
                  <div className="w-full h-full flex flex-col items-center justify-center p-4 relative">
                    <img src={pptxPreviewImages[currentSlide]} alt={`Slide ${currentSlide + 1}`} className="max-w-full max-h-full object-contain rounded-md shadow-md"/>
                    <div className="absolute top-2 right-2 flex items-center gap-2">
                       <span className="text-xs font-semibold text-gray-700 bg-white/70 backdrop-blur-sm rounded-full px-2 py-1">{currentSlide + 1} / {pptxPreviewImages.length}</span>
                       <button onClick={() => setPptxPreviewImages([])} className="p-1.5 rounded-full bg-black/40 hover:bg-black/60 text-white transition-colors">
                          <CloseIcon/>
                       </button>
                    </div>
                     {currentSlide > 0 && (
