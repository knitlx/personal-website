...
const GradientBorderButton = ({ children, className = '', useGradientText = false, ...props }: Props) => {
  const commonClassName = `inline-block rounded-lg p-[1px] bg-[linear-gradient(135deg,#AB5EED_50%,#7A68EE_80%)] shadow-button-cta transition-all duration-300 ease-in-out hover:-translate-y-0.5 no-underline ${className}`;
