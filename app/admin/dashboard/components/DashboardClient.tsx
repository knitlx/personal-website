...
import { ContentItem, GetAllContentResult } from "@/lib/content";
...
export default function DashboardClient({ userName, userEmail, projects, blogPosts }: DashboardClientProps) {
  ...
  const [projectSearchQuery, setProjectSearchQuery] = useState(currentSearchParams.get('projectsSearch') || '');
  const [blogSearchQuery, setBlogSearchQuery] = useState(currentSearchParams.get('blogSearch') || '');
      ...
      <div>
        {activeTab === 'projects' && (
          <div className="p-6 bg-white rounded-lg shadow-md">
            ...
            <Link href="/admin/projects/new" className="inline-block mt-4 px-4 py-2 bg-[#33239C] text-white font-semibold rounded-lg hover:bg-[#2A1E82]">
              Добавить проект
            </Link>
              ...
              {projects.data.length === 0 ? (
                <li className="text-gray-600">Пока нет проектов.</li>
                              ...
                              projects.data.map((project) => (
                                <li key={project.slug} className="grid grid-cols-[1fr_auto_auto] items-center gap-4 bg-gray-100 p-3 rounded-md">
                                  <span className="text-lg truncate">{project.title}</span>
                                    ...
                                    <Link href={`/admin/projects/edit/${project.slug}`} className="px-3 py-1 bg-[#20549B] text-white rounded-md hover:bg-[#1A4785]">
                                      Редактировать
                                    </Link>
                                    <button
            ...
            {renderPagination("projects", projects.totalPages, projects.currentPage, projectSearchQuery)}
          </div>
        )}
        ...
        {activeTab === 'blog' && (
          <div className="p-6 bg-white rounded-lg shadow-md">
            ...
            <Link href="/admin/blog/new" className="inline-block mt-4 px-4 py-2 bg-[#33239C] text-white font-semibold rounded-lg hover:bg-[#2A1E82]">
              Добавить статью
            </Link>
            <ul className="mt-4 space-y-2">
              {blogPosts.data.length === 0 ? (
                <li className="text-gray-600">Пока нет статей.</li>
                              ...
                              blogPosts.data.map((post) => (
                                <li key={post.slug} className="grid grid-cols-[1fr_auto_auto] items-center gap-4 bg-gray-100 p-3 rounded-md">
                                  <span className="text-lg truncate">{post.title}</span>
                                    ...
                                    <Link href={`/admin/blog/edit/${post.slug}`} className="px-3 py-1 bg-[#20549B] text-white rounded-md hover:bg-[#1A4785]">
                                      Редактировать
                                    </Link>
                                    <button
