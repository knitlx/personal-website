/**
 * Генерирует slug из заголовка с транслитерацией кириллицы
 */
export function generateSlug(title: string): string {
  const cyrillicToLatinMap: { [key: string]: string } = {
    а: "a",
    б: "b",
    в: "v",
    г: "g",
    д: "d",
    е: "e",
    ё: "yo",
    ж: "zh",
    з: "z",
    и: "i",
    й: "y",
    к: "k",
    л: "l",
    м: "m",
    н: "n",
    о: "o",
    п: "p",
    р: "r",
    с: "s",
    т: "t",
    у: "u",
    ф: "f",
    х: "h",
    ц: "ts",
    ч: "ch",
    ш: "sh",
    щ: "sch",
    Ъ: "",
    ы: "y",
    ь: "",
    э: "e",
    ю: "yu",
    я: "ya",
    А: "A",
    Б: "B",
    В: "V",
    Г: "G",
    Д: "D",
    Е: "E",
    Ё: "Yo",
    Ж: "Zh",
    З: "Z",
    И: "I",
    Й: "Y",
    К: "K",
    Л: "L",
    М: "M",
    Н: "N",
    О: "O",
    П: "P",
    Р: "R",
    С: "S",
    Т: "T",
    У: "U",
    Ф: "F",
    Х: "H",
    Ц: "Ts",
    Ч: "Ch",
    Ш: "Sh",
    Щ: "Sch",
    Ы: "Y",
    Э: "E",
    Ю: "Yu",
    Я: "Ya",
  };

  let processedTitle = title;

  // Transliterate Cyrillic first (before lowercase)
  processedTitle = Array.from(processedTitle)
    .map((char) => (char in cyrillicToLatinMap ? cyrillicToLatinMap[char] : char))
    .join("");

  // Convert to lowercase
  processedTitle = processedTitle.toLowerCase();

  // Remove non-alphanumeric characters (excluding hyphen)
  processedTitle = processedTitle.replace(/[^a-z0-9-]/g, "-");

  // Remove multiple consecutive hyphens
  processedTitle = processedTitle.replace(/-+/g, "-");

  // Remove leading/trailing hyphens
  return processedTitle.replace(/^-+|-+$/g, "");
}
