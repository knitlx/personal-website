...
export const blogPostSchema = z.object({
  ...
  title: z
    ...
    .trim(),
  date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, "Неверный формат даты. Используйте YYYY-MM-DD"),
  description: z
    .string()
    .min(10, "Описание должно содержать минимум 10 символов")
    .max(500, "Описание не должно превышать 500 символов")
    .trim(),
  articleBody: z.string().min(10, "Статья должна содержать минимум 10 символов").trim(),
